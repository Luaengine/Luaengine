...
url.segment_set = url.make_set {
    "-", "_", ".", "!", "~", "*", "'", "(",
	")", ":", "@", "&", "=", "+", "$", ",",
}

function url.protect_segment(s)
	return string.gsub(s, "([^A-Za-z0-9_])", function (c)
		if url.segment_set[c] then return c
...
-----------------------------------------------------------------------------
function url.absolute_path(base_path, relative_path)
...
-- Input
--   parsed: parsed URL, as returned by url.parse
-- Returns
    ...
    elseif relative_parsed.scheme then return relative_url
        ...
        if not relative_parsed.authority then
            ...
            if not relative_parsed.path then
                ...
                relative_parsed.path = url.absolute_path(base_parsed.path or "",
...
function url.build_path(parsed, unsafe)
	...
	if unsafe then
		...
		for i = 1, n-1 do
			path = path .. url.protect_segment(parsed[i])
			path = path .. "/"
		end
		if n > 0 then
			path = path .. url.protect_segment(parsed[n])
			if parsed.is_directory then path = path .. "/" end
		end
	end
